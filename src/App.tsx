...
const TopSongsChart = React.lazy(
  () => import("./components/charts/TopSongsChart")
);
...
export default function App() {
  ...
  const filteredStreams = useMemo(() => {
    return streams.filter((stream) => {
      if (selectedMonth) {
        const streamMonth = new Date(stream.dateStreamed).toLocaleString(
          "default",
  ...
  const handleMonthClick = useMemo(
    () => (month: string | null) => {
  ...
  const handleRevenueClick = useMemo(
    () => (source: string | null) => {
  ...
  const metricCards = useMemo(
    () => (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
        {metricCardDetails &&
          metricCardDetails.map((metric, i) => (
            <MetricCard
              key={i}
              title={metric.title}
              value={metric.value}
